    ...
    <div className="flex items-center gap-3">
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((s) => {
          ...
          return <Star key={s} className={`w-5 h-5 ${filled ? "text-primary fill-primary" : half ? "text-primary" : "text-white/20"}`} />;
        })}
      </div>
...
export default async function FilmPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  let publicReviews: { id: string; rating: number | null; review: string | null; watchedAt: Date; userId: string }[] = [];
  try {
    publicReviews = await db.select({
      id: watchedFilms.id,
      rating: watchedFilms.rating,
      review: watchedFilms.review,
      watchedAt: watchedFilms.watchedAt,
      userId: watchedFilms.userId,
    }).from(watchedFilms).where(
      and(eq(watchedFilms.tmdbId, movieId), isNotNull(watchedFilms.review))
    ).limit(10);
  } catch {
    // DB not configured
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex flex-col gap-4 flex-1">
              ...
              <div className="flex flex-wrap gap-2">
                {movie.genres.map((g) => (
                  <Badge key={g.id} variant="outline" className="border-primary/30 text-primary bg-primary/5">{g.name}</Badge>
                ))}
                {movie.release_date && <Badge variant="outline" className="border-border text-muted-foreground">{movie.release_date.slice(0, 4)}</Badge>}
                {movie.runtime > 0 && (
              ...
              <FilmActions
                ...
                initialWatched={filmStatus.watched}
                initialInWatchlist={filmStatus.inWatchlist}
                initialRating={filmStatus.watchedEntry?.rating}
                initialReview={filmStatus.watchedEntry?.review}
                isLoggedIn={!!session}
                locale={l}
              />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                {director && (
                  <div>
                    ...
                    <Link href={`/${l}/actor/${director.id}`} className="text-sm font-medium hover:text-primary transition-colors">{director.name}</Link>
                  </div>
                )}
                {movie.original_language && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-0.5">{t("language")}</p>
                    <p className="text-sm font-medium uppercase flex items-center gap-1"><Globe className="w-3 h-3" /> {movie.original_language}</p>
                  </div>
                )}
                {movie.budget > 0 && (
                  <div>
          ...
          <section className="px-6 py-8 max-w-7xl mx-auto w-full border-t border-border/50">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
              {topCast.map((actor) => {
                ...
                return (
                  <Link key={actor.id} href={`/${l}/actor/${actor.id}`} className="flex flex-col items-center text-center gap-2 group">
                    <div className="w-14 h-14 rounded-full overflow-hidden bg-muted ring-2 ring-border group-hover:ring-primary transition-colors shrink-0">
                      {avatarUrl ? (
                        <Image src={avatarUrl} alt={actor.name} width={56} height={56} className="object-cover w-full h-full" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-muted-foreground text-lg font-bold">{actor.name[0]}</div>
                      )}
                    </div>
                    <div>
                      <p className="text-xs font-medium leading-tight group-hover:text-primary transition-colors">{actor.name}</p>
        ...
        {/* Reviews */}
        <section className="px-6 py-8 max-w-7xl mx-auto w-full border-t border-border/50">
          <h2 className="text-xl font-bold mb-5">{t("reviews")}</h2>
          {publicReviews.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              {session ? t("noReviews") : t("loginToReview")}
            </p>
          ) : (
                  ...
                  <CardContent className="p-4">
                    {r.rating && (
                      <div className="flex gap-0.5 mb-2">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star key={s} className={`w-3.5 h-3.5 ${s <= r.rating! ? "text-primary fill-primary" : "text-white/20"}`} />
                        ))}
                      </div>
                    )}
                  ...
                  <Link key={film.id} href={`/${l}/film/${film.id}`}>
                    <Card className="border-border/50 bg-card card-hover cursor-pointer overflow-hidden group">
                      <div className="relative h-44">
                        {poster ? <Image src={poster} alt={film.title} fill className="object-cover" sizes="200px" /> : <div className="h-full flex items-center justify-center bg-muted"><Film className="w-8 h-8 text-muted-foreground" /></div>}
                      </div>
